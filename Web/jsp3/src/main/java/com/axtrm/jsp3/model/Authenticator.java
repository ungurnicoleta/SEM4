...
public class Authenticator {
    ...
    public void connect() {
        try {
            ...
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/Exam?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC", "root", "test");
    ...
    public Astronauts authenticate(String username) {
        ResultSet rs;
        Astronauts u = null;
        ...
        try {
            ...
            if (rs.next())
                u = new Astronauts(rs.getInt("ID"), rs.getString("name"), rs.getInt("civilizationID"));
            rs.close();
    ...
    public List<Expeditions> getExpeditions() {
        List<Expeditions> profiles = new ArrayList<Expeditions>();
        ResultSet rs;
        try {
            rs = stmt.executeQuery("select * from Expeditions");
            while (rs.next()) {
                profiles.add(new Expeditions(
                        rs.getInt("ID"),
                        rs.getInt("astronautID"),
                        rs.getInt("planetID"),
                        rs.getFloat("deathprobability")
                ));
            }
            rs.close();
        } catch (SQLException e) {
    ...
    public Planets getPlanetById(int id) {
        ...
        Planets c = null;
        try {
            rs = stmt.executeQuery("select * from Planets where id =" + id);
            if (rs.next())
                c = new Planets(
                        rs.getInt("id"),
                        rs.getString("name"),
